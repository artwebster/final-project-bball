...
const GameOdds = ({ gamePreDetails }) => {
  ...
  // if (!spreadAway) return <div>No market data available, sorry!</div>;
  ...
  return (
    <Wrapper>
      ...
      <>
        ...
        {(userInfo) ? <button value={spreadAway} onClick={(ev) => handleClick(ev, "spread", gamePreDetails.awayTeam.abbr)}>{spreadAway}</button> : <div>{spreadAway}</div>}
      ...
      <>
        ...
        {(userInfo) ? <button value={spreadHome} onClick={(ev) => handleClick(ev, "spread", gamePreDetails.homeTeam.abbr)}>{spreadHome}</button> : <div>{spreadHome}</div>}
